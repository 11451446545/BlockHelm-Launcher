...
public sealed class LauncherSelfUpdateService : ILauncherSelfUpdateService
{
    ...
    {
        if (!update.CanAutoInstall || update.SizeBytes > OfficialUpdateHttp.MaximumExecutableBytes)
    ...
    private string ResolveUpdateDirectory(string version)
    {
        var updatesRoot = Path.GetFullPath(Path.Combine(
            ...
            "cache",
            "updates"));
        var segment = SanitizePathSegment(version);
        var updateDirectory = Path.GetFullPath(Path.Combine(updatesRoot, segment));
        var relativePath = Path.GetRelativePath(updatesRoot, updateDirectory);
        if (relativePath == "."
            || relativePath == ".."
            || relativePath.StartsWith($"..{Path.DirectorySeparatorChar}", StringComparison.Ordinal)
            || Path.IsPathRooted(relativePath))
        {
            throw new UpdateSecurityException("The update version produced an unsafe cache path.");
        }
    ...
    private static string SanitizePathSegment(string value)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new UpdateSecurityException("The update version is invalid.");
        var invalid = Path.GetInvalidFileNameChars();
        var sanitized = new string(value.Select(character => invalid.Contains(character) ? '_' : character).ToArray());
        var windowsNormalized = sanitized.TrimEnd(' ', '.');
        if (string.IsNullOrWhiteSpace(windowsNormalized) || windowsNormalized is "." or "..")
            throw new UpdateSecurityException("The update version is invalid.");
        return sanitized;
    }
